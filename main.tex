\documentclass[10pt,twoside,cucitura]{toptesi}
\usepackage{lipsum}
\usepackage{hyperref}
\usepackage{tabularx}
\usepackage{framed}
\usepackage{amsmath}
\usepackage{svg}
\usepackage{graphicx}
\usepackage{float}
\usepackage{array}
\usepackage{enumitem}
\usepackage[babel]{csquotes}
\usepackage[isbn=true,hyperref]{biblatex}

\hypersetup{%
    pdfpagemode={UseOutlines},
    bookmarksopen,
    pdfstartview={FitH},
    colorlinks,
    linkcolor={blue},
    citecolor={red},
    urlcolor={blue}
  }

\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}\usepackage{lmodern}

\ateneo{ITST J.F. Kennedy}
\nomeateneo{Pordenone}
\FacoltaDi{Specializzazione Informatica}
\titolo{Lupus in Tabula}
\sottotitolo{Limiti della Prima Forma Normale}

\candidato{\tabular{@{}c@{}}Edoardo \textsc{Morassutto}\\Quinta C IA\endtabular}

\sedutadilaurea{\textsc{Anno~scolastico} 2015-2016}
\logosede{logo}

\addbibresource{bibliografia.bib}
\input{functions.tex}

\begin{document}

\frontespizio

\sommario
\input{sommario.tex}

\indici

\mainmatter

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                  PARTE UNO                          %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\part{Lupus in Tabula}
\label{part:part1}

\chapter{Introduzione generale}

\section{Descrizione del gioco}
\input{parte1/01.descrizione.tex}

\section{Ruoli}
\input{parte1/02.ruoli.tex}

\chapter{Analisi}

\section{Analisi del problema}
\input{parte1/03.analisi.tex}

\section{Struttura dei componenti}
\input{parte1/04.componenti.tex}

\section{Progettazione della base di dati}
\input{parte1/05.database.tex}

\subsection{Schema concettuale}
\input{parte1/06.ER.tex}

\subsection{Schema logico}
\input{parte1/07.schemaLogico.tex}

\subsection{Descrizione delle entità}
\input{parte1/09.attributi.tex}

\section{Codici utilizzati}
\input{parte1/10.codici.tex}

\subsection{Tempo del gioco}
\label{sec:tempo}
\input{parte1/11.tempo.tex}

\subsection{Stato della partita}
\label{sec:statoPartita}
\input{parte1/12.statoPartita.tex}

\subsection{Codici di risposta delle API}
\input{parte1/13.statoAPI.tex}

\subsection{Codici degli eventi}
\label{sec:codiciEventi}
\input{parte1/14.statoEventi.tex}

\subsection{Visibilità della stanza}
\label{sec:visibilita}
\input{parte1/15.visibilita.tex}

\subsection{Stato dei giocatori}
\label{sec:statoGiocatore}
\input{parte1/16.statoGiocatore.tex}

\chapter{Implementazione}

\section{Tecnologie utilizzate}
\input{parte1/17.tecnologie.tex}

\section{Configurazione}
\input{parte1/18.configurazione.tex}

\section{Pagine web}
\label{sec:pagineWeb}
\input{parte1/19.pagineWeb.tex}

\section{Webservice REST}
\input{parte1/20.webservice.tex}

\section{Polimorfismo dei ruoli}
\label{sec:polimorfismo}
\input{parte1/21.polimorfismo.tex}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                  PARTE DUE                          %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\part{Limiti della 1NF}

\chapter{Introduzione alle forme normali}

\section{Cenni introduttivi}
\input{parte2/01.introduzione.tex}

\section{Prima forma normale}
\input{parte2/02.1NF.tex}

\section{Seconda forma normale}
\input{parte2/03.2NF.tex}

\section{Terza forma normale}
\input{parte2/04.3NF.tex}

\section{Forma normale di Boyce-Codd}
\input{parte2/05.BCNF.tex}

\section{Altre forme normali}
\input{parte2/06.altreNF.tex}

\chapter{Problematiche con la 1NF}

\section{Contesto del problema}
\input{parte2/07.contesto.tex}

\section{Prime soluzioni parziali}
\input{parte2/08.soluzioniParziali.tex}

\subsection{Infrangere la prima forma normale}
\label{sec:soluzione1}
\input{parte2/09.soluzione1.tex}

\subsection{Una tabella per tipo}
\input{parte2/10.soluzione2.tex}

\subsection{Tabelle key-value}
\input{parte2/11.soluzione3.tex}

\section{Soluzione proposta}
\label{sec:soluzione}
\input{parte2/12.soluzione.tex}

\cleardoublepage
\phantomsection
\addcontentsline{toc}{chapter}{\bibname}
\printbibliography

\end{document}
